...
function buildPrompt(seg: Segment, bars: BarItem[], projectName: string): string {
  ...
  return [
    `Produce an UNVERIFIED AI CANDIDATE SKETCH of a reinforced-concrete element — schematic only, NOT a fabrication-ready drawing. Style: loose CAD-like linework, clearly marked as a candidate.`,
    ...
    `• Right ~20%: a small LEGEND box only (hatched embed plate, hatched bearing-plate pocket, OWSJ bearing plate). DO NOT draw any title block, drawing number, scale field, detailer/checker/approver rows, or revision cell — the host application adds an "AI CANDIDATE" header outside the image.`,
    ``,
    `DRAWING CONTENT (schematic only — do NOT imply measured accuracy):`,
    `• Approximate dimension chains along top/bottom edges. Use round numbers (e.g. 500, 2600, 7500). Do not present them as final.`,
    `• Grid bubbles: circles with letters/numbers (A1, A2 … on top; AA, AB … on right) connected by thin dashed grid lines.`,
    `• Rebar shown as solid black lines with GENERIC mark labels only (e.g. "C1", "T1", "SW1"). DO NOT write quantity-length triplets like "C1 32 - 20M x 4500"; DO NOT render a bar list table inside the image.`,
    `• Bar marks that may appear (labels only, no quantities): ${marks || "C1, T1, SW1"}.`,
    `• Bar sizes used (metric Canadian) shown as generic spacing notes only (e.g. "20M EW"): ${sizes || "10M, 15M, 20M"}.`,
    ...
    `• For any change callout, draw a DASHED ORANGE balloon with the literal label "Candidate #1" (incrementing). DO NOT draw a revision cloud, DO NOT draw a revision triangle, DO NOT write the literal strings "REV", "REV.", "R0", "R1", "DETAILER", "CHECKER", "CHECKED", "APPROVED", "ISSUED FOR CONSTRUCTION", or any title-block field.`,
    `• Optional small "AI Note" tag (italic) near a callout is allowed; "REV" / revision letters are NOT.`,
    ``,
    `STYLE — non-negotiable:`,
    `• Looks like a SCHEMATIC AutoCAD-style sketch. Crisp 0.13–0.5 mm linework, monospaced/Arial Narrow text, all caps for labels.`,
    `• Pure white background, primarily black ink, with the two accent hatches above plus dashed orange for candidate-change balloons. NO 3-D, NO perspective, NO shading, NO colour fills beyond those, NO photorealism, NO hand-sketch look.`,
    `• High-resolution vector-clean lines, anti-aliased text. Treat the output as a CANDIDATE for human review, not as an issued drawing.`,
  ].join("\n");
}
