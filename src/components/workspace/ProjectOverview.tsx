...
import { getCanonicalProjectSummary, type CanonicalProjectView } from "@/lib/rebar-read-model";
...
export default function ProjectOverview({ project }: ProjectOverviewProps) {
  ...
  useEffect(() => {
    ...
    const loadCounts = async () => {
      ...
      try {
        canonicalSummary = await getCanonicalProjectSummary(supabase, project.id);
      } catch (error) {
        console.warn("Failed to load canonical project summary:", error);
      }

      const [f, s, i, a, e, b] = await legacyCountsPromise;
      if (cancelled) return;

      setCounts({
        files: canonicalSummary?.fileCount ?? f.count ?? 0,
        segments: s.count || 0,
        issues: i.count || 0,
        approvals: a.count || 0,
        estimates: canonicalSummary?.estimateVersionCount ?? e.count ?? 0,
        blockers: b.count || 0,
      });
    };
